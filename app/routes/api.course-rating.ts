...
import { upsertCourseRating, isUserEnrolledInCourse } from "~/services/reviewService";
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (!isUserEnrolledInCourse(currentUserId, courseId)) {
    throw data("You must be enrolled to rate this course", { status: 403 });
  }

  upsertCourseRating(currentUserId, courseId, rating);
